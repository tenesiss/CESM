...
\begin{align}
h_f^{\mathrm{fr}}&=\LN_t(\bar v_f+d_f),\\
...
\end{align}
For $f\in\mathcal W_i$, the supervised frame label is $y_i$.
...
\label{eq:token-ce}
\end{equation}
For frame prediction, let
$\mathcal J=\{(b,f):f\in\bigcup_{i=1}^{N_b}\mathcal W_{bi}\}$ and
let $y_{b,f}^{\mathrm{fr}}$ be the unique token whose window contains
$f$. The loss becomes
\begin{equation}
\mathcal L_{\mathrm{pred}}
=-\frac{1}{|\mathcal J|}
