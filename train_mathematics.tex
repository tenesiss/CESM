...
The previous-token term uses a separate distribution
$p_{bif}^{\mathrm{prev}}$: reuse the same encoded video through frame $f$,
but restrict video-to-text attention to $\BOS,y_{b1},\ldots,y_{b,i-2}$.
The usual $p_{bf}^{\mathrm{fr}}$ can attend through $y_{b,i-1}$.
Excluding the previous target and all states that encode it prevents the
auxiliary term from learning to copy an already visible teacher token.
The causal text encoder makes earlier states reusable under either mask.
...
-\tfrac12\log p_{bif}^{\mathrm{prev}}(y_{b,i-1}),&f\in\mathcal P_{bi},\\
-\log p_{bf}^{\mathrm{fr}}(y_{bi}),&f\notin\mathcal P_{bi}.
\end{cases}
\label{eq:phased-frame-ce}
\end{equation}
This averages two cross entropies with different conditioning prefixes;
it is not a soft target applied to one distribution. Equal consecutive
token IDs still require both terms because their text contexts differ.
The implementation shares encoder outputs and attention Q/K/V projections,
evaluates the extra attention and token head only on phased frames, and
checkpoints chunks of at most 128 frames through cross entropy. This bounds
the extra vocabulary activations without rerunning either encoder.
...
Phasing does not shift alignment boundaries or the current-token path's
teacher-token availability in \eqref{eq:v2t}; only the auxiliary path
removes the latest available text state. Stage~II retains the same
