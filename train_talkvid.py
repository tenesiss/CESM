#!/usr/bin/env python3
"""Download TalkVid clips, align them with the local frame grouper, then train.
...
unique source YouTube uploads. No TalkVid model code or weights are required.
"""
...
def build_argparser():
    ...
    group = parser.add_argument_group("TalkVid download and preprocessing")
    group.add_argument("--num-videos", "-N", type=positive_int,
                       help="Number of successfully aligned TalkVid clips to train on; required "
                            "except with --login or --logout")
    group.add_argument("--work-dir", type=Path, default=ROOT / "data" / "talkvid")
    ...
    group.add_argument("--start-index", type=int, default=0,
                       help="Skip this many metadata rows before filtering (default: 0)")
...
def login_cookie_path(args):
    return args.work_dir.expanduser().resolve() / ".auth" / "youtube.cookies.txt"
    ...
    if "ffmpeg exited with code" in lower:
        ...
        if option_errors:
            return DownloadSetupError(
                ...
                "Use the updated train_talkvid.py or update FFmpeg. "
...
def probe_video(path, expected_duration, max_frames=None):
    ...
    if max_frames is not None:
        ...
        if frames == max_frames:
            ...
            expected_duration = min(expected_duration, video_duration)
...
def prepare(args):
    ...
    report = {"metadata": args.metadata, "requested": args.num_videos,
    ...
    print(f"Reading metadata: {args.metadata}", flush=True)
    try:
        with partial.open("w", encoding="utf-8") as jsonl, contextlib.closing(iter_metadata(args.metadata)) as rows:
            for index, row in enumerate(rows):
                ...
                print(f"[{len(records)}/{args.num_videos} usable; attempt {attempts}/{args.max_attempts}] "
                      f"{clip.url} [{clip.start:.3f}, {clip.end:.3f}]", flush=True)
                try:
                    ...
                    if isinstance(exc, (WhisperSetupError, DownloadSetupError)):
                        raise
        ...
        if len(records) != args.num_videos:
            raise RuntimeError(
                ...
                "Increase --max-attempts or change the metadata/filter. Completed clips are cached."
            )
...
def validate_args(parser, args):
    ...
    missing = [name for name in ("yt_dlp", "ijson", "faster_whisper")
               if importlib.util.find_spec(name) is None]
    if missing:
        parser.error(f"Missing dependencies: {', '.join(missing)}. Install with: "
                     f"{sys.executable} -m pip install -r {ROOT / 'requirements-talkvid.txt'}")
